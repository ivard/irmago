...
type redPacket struct {
	...
	RecoveryKeyPair *recoveryKeyPair `json:"recoveryKeyPair"`
	DeviceKeyPair	*deviceKeyPair   `json:"deviceKeyPair"`
...
type deviceKeyPair struct {
	...
	privateKey      [32]byte         `json:"privateDeviceKey"`
}
...
type recoveryRequest struct {
	RedPacketBytes	[]byte			`json:"redPacket"`
	Delta           []byte			`json:"delta"`
}

// Recovery PIN uses private recovery key in the hash
func (rp *redPacket) HashedPin(pin string) string {
	hash := sha256.Sum256(append(rp.RecoveryNonce[:], []byte(pin)...))
...
func (rs *recoverySession) verifyPinAttempt(pin string) (
	...
	pinmsg := keysharePinMessage{Username: rs.RedPacket.KeyshareServer.Username, Pin: rs.RedPacket.HashedPin(pin)}
	...
	case kssPinSuccess:
		rs.RedPacket.KeyshareServer.token = pinresult.Message
		rs.transport.SetHeader(kssUsernameHeader, rs.RedPacket.KeyshareServer.Username)
		rs.transport.SetHeader(kssAuthHeader, rs.RedPacket.KeyshareServer.token)
	case kssPinFailure:
...
func (rs *recoverySession) renewDeviceKeys() {
	...
	rr := recoveryRequest{rs.RedPacketBytes, deltaBytes[:]}
	rs.RedPacket = &redPacket{}
	rs.transport.Post("users/recovery/new-device", rs.RedPacket, rr)
...
func (rp *redPacket) curveEncrypt (plain []byte) (ciphertext []byte) {
	var nonce [24]byte
	if _, err := io.ReadFull(rand.Reader, nonce[:]); err != nil {
		panic(err)
	}
	...
	ciphertext = box.Seal(nonce[:], msg, &nonce, &rp.RecoveryKeyPair.PublicKey, &rp.DeviceKeyPair.privateKey)
	return
}

func (rp *redPacket) curveDecrypt (ciphertext []byte) (plain []byte) {
	var decryptNonce [24]byte
	copy(decryptNonce[:], ciphertext[:24])
	plain, ok := box.Open(nil, ciphertext[24:], &decryptNonce, &rp.DeviceKeyPair.PublicKey, &rp.RecoveryKeyPair.privateKey)
	if !ok {
...
func startRecovery(handler recoverySessionHandler, storage *storage) {
	...
	servers, rp, bp := parseBackup(backup)

	for _, kss := range servers {
		transport := irma.NewHTTPTransport(kss.URL)

		rs := &recoverySession{nil, rp, bp,
			handler, pin, transport, storage}
...
func parseBackup(backup *[]byte) (ksses []keyshareServer, redPacket, bluePacket []byte) {
	//TODO
	return nil, nil, nil
}
